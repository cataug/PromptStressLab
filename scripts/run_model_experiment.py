...
def load_model(model_id: str, model_path: Path) -> Any:
    ...
    device_map_mode = os.environ.get(
        "PSL_DEVICE_MAP",
        "auto",
    )

    if device_map_mode == "full":
        device_map = {"": 0}
    else:
...
def context_limit(model: Any, tokenizer: Any) -> int:
    """
    Return the model's global context length.

    sliding_window is deliberately ignored: for Gemma-3 it is the
    local-attention window, not the model's total context capacity.
    """
